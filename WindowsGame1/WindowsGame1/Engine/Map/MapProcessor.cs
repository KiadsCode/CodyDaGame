...
namespace WindowsGame1.Engine.Map
{
    public class MapProcessor
    {
        ...
        public static MapData Process(string mapPath)
        {
            MapData map = new MapData();
            string[] fileText = GetFileData(mapPath);
            ...
            foreach (string mapSegment in fileText)
            {
                ...
                for (int i = 0; i < preParsedPosition.Length; i++)
                    preParsedPosition[i] = preParsedPosition[i].Trim();
                Dictionary<string, string> typesExceptions = new Dictionary<string, string>();
                typesExceptions.Add("Player", "3");
                typesExceptions.Add("SolidBlock", "0");
                typesExceptions.Add("WeakBlock", "1");
                typesExceptions.Add("Enemy", "2");
                try
                {
                    type = Convert.ToInt32(preParsedType);
                }
                ...
                {
                    type = Convert.ToInt32(typesExceptions[preParsedType]);
                }
                position = new Vector2(
                    Convert.ToInt32(preParsedPosition[0]),
                    Convert.ToInt32(preParsedPosition[1]));
        ...
        private static string[] GetFileData(string mapPath)
        {
            string[] data = new string[1];
            string rawData = string.Empty;
